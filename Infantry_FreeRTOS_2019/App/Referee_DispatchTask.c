...
void DispchRefereeTask(void *parmas)
{
	...
	for(;;)
	{
		next:	Len = USART6_GetDataCount();
		
		if(Len >= 10)
		{ 
			if(	USART6_At(0)  == 0xA5 )
			{
				frame_len =  USART6_At(2) << 8 | USART6_At(1);  
				if(Len >= frame_len)
				{
					 USART6_Recv(Transmission_BufferOfUsart6,frame_len + 9);
				}
				else 
				{
				    vTaskDelay(5);
				...
				if(Verify_CRC8_Check_Sum(Transmission_BufferOfUsart6,5) &&  Verify_CRC16_Check_Sum(Transmission_BufferOfUsart6,frame_len + 9) == true) 
				{
					...
					Referee_date1.CmdID  =  Transmission_BufferOfUsart6[5] | Transmission_BufferOfUsart6[6] << 8;
